...
const QuestionTypes = props => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setQuestionId(props.navigation.state.params.data.id);
      var ans = [];
      if (props.navigation.state.params.data.ans_data != null) {
  ...
  useEffect(() => {
    if (
      updateUserQuestionRes != undefined &&
      updateUserQuestionRes.data != undefined
    ) {
  ...
  const singleChoiceApiCall = () => {
    ...
    // console.log("single sendData",sendData)
    dispatch(updateUserQuestionApi(sendData));
  };
...
const mapStateToProps = state => {
  // console.log("Edit answer Card state", state)
  return {};
};

export default connect(mapStateToProps, {})(QuestionTypes);
