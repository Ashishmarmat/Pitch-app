...
const CreateEducationScreen = props => {
  const dispatch = useDispatch();
  const AddEducationRes = useSelector(
    state => state.EducationListReducer.addEducationData,
  );
  ...
  useEffect(() => {
    if (
      AddEducationRes &&
      AddEducationRes != undefined &&
      AddEducationRes.response_code &&
      AddEducationRes.response_code === 200
    ) {
  ...
  const createTwoButtonAlert = () =>
    Alert.alert('Added education successfully', '', [
      {text: 'OK', onPress: () => goToNext()},
    ]);

  const goToNext = () => {
    dispatch(addUserEducationSuccess());
    ...
    return (
      <View style={styles.headerMainView}>
        ...
        <View style={styles.titleMainView}>
          <Text style={styles.titleTextStyle}>Add Education</Text>
        </View>
        <TouchableOpacity
          onPress={() => AddEducationFunc()}
  ...
  const AddEducationFunc = () => {
    ...
    if (startDate > endDate) {
      ...
      dispatch(AddUserEduApi(SendData));
    }
  };
      ...
      <View style={{flex: 1, backgroundColor: '#fff'}}>
        <KeyboardAvoidingView
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={{paddingHorizontal: 15}}>
              <Text style={styles.startEndTextStyle}>Activities</Text>
              <CustomTextInput
                style={styles.titleTextInputStyle}
                label={''}
                placeholderTextColor={'#C4C4C4'}
...
const mapStateToProps = state => {
  // console.log("create Educ state", state)
  return {};
};

export default connect(mapStateToProps, {})(CreateEducationScreen);
